...
class MyAccountTest03Ninja(unittest.TestCase):
    ...
    def test_my_account_03_ninja(self):
        ...
        assert element, "ERROR. 'Register for an affiliate account' link is not displayed."
        # Click on registration affiliate account link
        myacc.click_reg_aff_acc_link()
        # Verify if registration affiliate account link is working
        element = self.driver.find_element_by_xpath(MyAccountNinja.verify_reg_aff_acc).is_displayed()
        assert element, "ERROR. 'Register for an affiliate account' link is broken."
